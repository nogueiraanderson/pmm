...
# default JVM (remoting floor since Jenkins 2.555.1), Java 17 still present for
# jobs that address it by path, a working Docker daemon usable by ec2-user, the
# SSM agent registered, cloud-init finished. This template only validates. The
# workflow deregisters a failed candidate and promotes a passing one in a
# separate, retried step.
#
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "rpm -q java-17-openjdk-headless >/dev/null || { echo 'java-17 no longer installed (jobs addressing it by path would break)'; exit 1; }",
      ...
      "docker run --rm oraclelinux:9 true || { echo 'cannot run a pre-pulled container'; exit 1; }",
      ...
      "for tool in kubectl helm eksctl doctl yq aws node docker-compose bats; do command -v \"$${tool}\" >/dev/null || { echo \"missing tool: $${tool}\"; exit 1; }; done",

...
packer {
  required_plugins {
    ...
    ansible = {
      version = "1.1.6" # exact pin, supply chain
...
build {
  ...
  provisioner "shell" {
    timeout = "10m"
    inline = [
      "sudo cloud-init status --wait 2>/dev/null | grep -qE 'done|disabled' || { echo 'cloud-init not done'; sudo cloud-init status --long; exit 1; }",
    ]
  }
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      # modules keep running under the system python.
      "sudo dnf install -y -q python3.12 python3.12-pip",
      "sudo python3.12 -m pip install --quiet \"ansible-core==$${ANSIBLE_CORE_VERSION}\"",
      ...
      "ansible-playbook --version > /tmp/ansible-version.txt",
      "grep -F \"core $${ANSIBLE_CORE_VERSION}\" /tmp/ansible-version.txt",
      "cd /tmp/ansible && ansible-galaxy collection install -r requirements.yml",
      ...
      "cd /tmp/ansible && ansible-playbook -i local-inventory agent-aws.yml",
    ]
  }
